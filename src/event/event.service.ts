...
export class EventService {
    ...
    async createEvent(payload: CreateEventPayload): Promise<EventDto> {
        const isHostExist = await this.eventRepository.getUserById(
            payload.hostId,
        );
        if (!isHostExist) {
        ...
        const isCityExist = await this.eventRepository.getCityById(
            payload.cityId,
        );
        if (!isCityExist) {
    ...
    async joinEvent(eventId: number, payload : UpdateEventJoinPayload) : Promise<void> {
        ...
        const isUserExist = await this.eventRepository.getUserById(
            payload.userId,
        );
        if((!isUserExist)) {
        ...
        const isEventJoin = await this.eventRepository.isUserJoinedEvent(eventId, payload.userId);
        if (isEventJoin) {
    ...
    async outEvent(eventId: number, payload : UpdateEventJoinPayload) : Promise<void> {
        ...
        const eventJoin = await this.eventRepository.isUserJoinedEvent(eventId, payload.userId);
        if (!eventJoin) {
